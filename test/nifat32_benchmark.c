...
timer_t write_timer;
timer_t read_timer;
timer_t rename_timer;
timer_t copy_timer;
timer_t delete_timer;
...
int main(int argc, char* argv[]) {
    ...
    while (count-- > 0) {
        ...
        if (!(count % 1000)) {
            ...
            reset_timer(&open_timer);
            reset_timer(&write_timer);
            reset_timer(&read_timer);
            reset_timer(&rename_timer);
            reset_timer(&copy_timer);
            reset_timer(&delete_timer);
        }
